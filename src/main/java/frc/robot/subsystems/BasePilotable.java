...
public class BasePilotable extends SubsystemBase {
  ...
  public BasePilotable() {
    ...
    conversionEncodeur = Math.PI * 0.2032 / (256 * 3 * 2.5);// Ancienne valeur 0.1865 //roue de 18.46 cm déterminé
                                                            // manuellement, ratio 2.5:1 shaft-roue 3:1 encodeur-shaft
                                                            // encodeur 256 clic encodeur circonférence de la roue 58cm
                                                            // aproximatif
    ...
    // getPositionG(), getPositionD());

    poseEstimator = new DifferentialDrivePoseEstimator(Constants.kinematics, Rotation2d.fromDegrees(getAngle()),
  ...
  public void periodic() {
    ...
    // odometry.update(Rotation2d.fromDegrees(getAngle()), getPositionG(),
    // getPositionD());
  ...
  public void resetOdometry(Pose2d pose) {
    ...
    // odometry.resetPosition(Rotation2d.fromDegrees(getAngle()), getPositionG(),
    // getPositionD(), pose);
  }
  ...
  public PathPlannerTrajectory creerTrajectoirePathPlanner(String trajet, boolean reversed) {
    return PathPlanner.loadPath(trajet, Constants.maxVitesse, Constants.maxAcceleration, reversed);
  ...
  public PathPlannerTrajectory creerTrajectoire(double x, double y, double angle) {
    PathPlannerTrajectory trajectoire = PathPlanner.generatePath(
        new PathConstraints(Constants.maxVitesse, Constants.maxAcceleration),
